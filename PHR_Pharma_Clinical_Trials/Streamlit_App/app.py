...
<div style="display:flex; align-items:center; margin-bottom:15px">
    ...
    <div>
        ...
        <p style="font-size:1.1rem; color:gray; margin:0; padding:0">Fivetran and Databricks-powered Streamlit data application for Clinical Trials</p>
    </div>
</div>
''', unsafe_allow_html=True)
...
UC_SCHEMA = os.environ.get("UC_SCHEMA", "dbx_connector_dbx_pharma")
UC_TABLE = os.environ.get("UC_TABLE", "phr_records")
...
# Define models list for dropdown
MODELS = [
    DBX_ENDPOINT,
    DBX_ENDPOINT_2,
    DBX_ENDPOINT_3,
    ...
    try:
        ...
        payload = {
            "messages": [
                {
                    "role": "system",
                    "content": "You are a pharmaceutical analytics expert analyzing clinical trial design and optimization data. Provide clear, evidence-based analysis with actionable insights for pharmaceutical companies and clinical trial teams."
                },
                {
                    "role": "user",
                    "content": prompt
...
def create_metrics_charts(data):
    ...
    if 'patient_age' in data.columns:
        try:
            data_copy = data.copy()
            data_copy['patient_age'] = pd.to_numeric(data_copy['patient_age'], errors='coerce')
            data_copy = data_copy.dropna(subset=['patient_age'])
            
            if not data_copy.empty:
                age_chart = alt.Chart(data_copy).mark_bar().encode(
    ...
    if 'enrollment_rate' in data.columns and 'disease_area' in data.columns:
        try:
            data_copy = data.copy()
            data_copy['enrollment_rate'] = pd.to_numeric(data_copy['enrollment_rate'], errors='coerce')
            
            # Remove rows where enrollment_rate is NaN or disease_area is null
            data_copy = data_copy.dropna(subset=['enrollment_rate', 'disease_area'])
            data_copy = data_copy[data_copy['disease_area'].str.strip() != '']  # Remove empty strings
            
            if not data_copy.empty and len(data_copy['disease_area'].unique()) > 0:
                enrollment_chart = alt.Chart(data_copy).mark_boxplot().encode(
    ...
    if 'dropout_rate' in data.columns and 'enrollment_date' in data.columns:
        try:
            # Convert enrollment_date to datetime and dropout_rate to numeric
            data_copy = data.copy()
            data_copy['enrollment_date'] = pd.to_datetime(data_copy['enrollment_date'], errors='coerce')
            data_copy['dropout_rate'] = pd.to_numeric(data_copy['dropout_rate'], errors='coerce')
            data_copy = data_copy.dropna(subset=['enrollment_date', 'dropout_rate'])
            ...
            if 'trial_status' in data.columns:
                try:
                    data_copy = data.copy()
                    data_copy['dropout_rate'] = pd.to_numeric(data_copy['dropout_rate'], errors='coerce')
                    data_copy = data_copy.dropna(subset=['dropout_rate'])
                    
                    if not data_copy.empty:
                        status_dropout_chart = alt.Chart(data_copy).mark_bar().encode(
    ...
    if 'site_name' in data.columns and 'enrollment_rate' in data.columns:
        try:
            # Convert enrollment_rate to numeric and group by site
            data_copy = data.copy()
            data_copy['enrollment_rate'] = pd.to_numeric(data_copy['enrollment_rate'], errors='coerce')
            
            # Group by site and calculate average enrollment rate for top 10 sites
            site_data = data_copy.groupby('site_name')['enrollment_rate'].agg(['mean', 'count']).reset_index()
            
            # Convert mean to numeric and filter out NaN values
            site_data['mean'] = pd.to_numeric(site_data['mean'], errors='coerce')
            site_data = site_data.dropna(subset=['mean'])
            
            # Only sites with 3+ patients and valid data
            site_data = site_data[site_data['count'] >= 3]
            
            if not site_data.empty:
                # Sort by mean enrollment rate and get top 10
                site_data = site_data.sort_values('mean', ascending=False).head(10)
            ...
            # If site performance analysis fails, skip this chart
            pass
...
# Ensure enrollment_rate and dropout_rate are included in numeric_candidates even if they're object type
for rate_col in ['enrollment_rate', 'dropout_rate', 'patient_age']:
    if rate_col in data.columns and rate_col not in numeric_candidates:
        # Test if the column can be converted to numeric
        try:
            test_series = pd.to_numeric(data[rate_col], errors='coerce')
            if not test_series.isna().all():  # If at least some values can be converted
                numeric_candidates.append(rate_col)
        except:
            pass
...
with tabs[0]:
    ...
    if numeric_candidates:
        # Convert object-type numeric columns to proper numeric before describe()
        data_for_stats = data[numeric_candidates].copy()
        for col in numeric_candidates:
            if data_for_stats[col].dtype == 'object':
                data_for_stats[col] = pd.to_numeric(data_for_stats[col], errors='coerce')
        
        # Create enhanced summary statistics
        summary_stats = data_for_stats.describe()
        ...
        with col1:
            ...
            # Always ensure enrollment_rate and dropout_rate are shown, even if not in summary_df
            if 'enrollment_rate' not in key_metrics_present and 'enrollment_rate' in data.columns:
                enroll_series = pd.to_numeric(data['enrollment_rate'], errors='coerce')
                enroll_mean = enroll_series.mean()
                enroll_min = enroll_series.min()
                enroll_max = enroll_series.max()
                st.metric(
                    label="Enrollment Rate",
                    value=f"{enroll_mean:.3f}",
                    help=f"Range: {enroll_min:.3f} - {enroll_max:.3f}"
                )
            
            if 'dropout_rate' not in key_metrics_present and 'dropout_rate' in data.columns:
                dropout_series = pd.to_numeric(data['dropout_rate'], errors='coerce')
                dropout_mean = dropout_series.mean()
                dropout_min = dropout_series.min()
                dropout_max = dropout_series.max()
                st.metric(
                    label="Dropout Rate",
                    value=f"{dropout_mean:.3f}",
                    help=f"Range: {dropout_min:.3f} - {dropout_max:.3f}"
                )
        ...
        with col2:
            ...
            if 'patient_age' in summary_df.index:
                ...
                # Fallback calculation if not in summary_df
                if 'patient_age' in data.columns:
                    age_series = pd.to_numeric(data['patient_age'], errors='coerce')
            ...
            if 'enrollment_rate' in summary_df.index:
                ...
                if 'enrollment_rate' in data.columns:
                    er_series = pd.to_numeric(data['enrollment_rate'], errors='coerce')
                    er_q75 = er_series.quantile(0.75)
                    er_q25 = er_series.quantile(0.25)
                    iqr = er_q75 - er_q25
                    insights.append(f"• **Enrollment Rate IQR**: {iqr:.3f}")
            ...
            if 'dropout_rate' in summary_df.index:
                ...
                # Fallback calculation
                if 'dropout_rate' in data.columns:
                    dr_series = pd.to_numeric(data['dropout_rate'], errors='coerce')
                    dr_median = dr_series.median()
                    dr_max = dr_series.max()
                    insights.append(f"• **Median Dropout Rate**: {dr_median:.3f}")
                    if dr_max > 30:  # Adjusted for the 99.960 value seen in your data
                        insights.append(f"• **⚠️ High dropout events**: up to {dr_max:.3f}")
        ...
        # Fallback if no numeric candidates found - ensure we always show the summary section
        st.subheader("📈 Summary Statistics")
        col1, col2 = st.columns(2)
        ...
        with col1:
            ...
            if 'patient_age' in data.columns:
                age_series = pd.to_numeric(data['patient_age'], errors='coerce')
                age_mean = age_series.mean()
                st.metric("Patient Age", f"{age_mean:.3f}")
            
            if 'enrollment_rate' in data.columns:
                enroll_series = pd.to_numeric(data['enrollment_rate'], errors='coerce')
                enroll_mean = enroll_series.mean()
                st.metric("Enrollment Rate", f"{enroll_mean:.3f}")
            
            if 'dropout_rate' in data.columns:
                dropout_series = pd.to_numeric(data['dropout_rate'], errors='coerce')
                dropout_mean = dropout_series.mean()
                st.metric("Dropout Rate", f"{dropout_mean:.3f}")
            ...
            if insights:
                ...
                with st.expander(f"📋 Generated {focus_area} Report (Real Clinical Trial Data)", expanded=True):
                    ...
                    <div class="agent-report-header">
                        ...
                        <small>Data Source: Live Databricks Clinical Trial Analysis</small><br>
